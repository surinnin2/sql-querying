...
SELECT * FROM analytics WHERE price > .10 AND price < 1 ORDER BY reviews DESC LIMIT 10;
...
SELECT app_name, reviews, min_installs, min_installs/reviews as proportion FROM analytics WHERE min_installs > 10000 AND min_installs/reviews = (SELECT MAX(min_installs/reviews) FROM analytics);
